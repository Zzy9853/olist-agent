# CLAUDE.md — Olist 智能问数 Agent

> 面向 Claude Code 的项目指南。本项目的双重目标：
> 1. **开发质量**：交付生产级思维的数据分析 Agent（安全、可评测、可深挖）
> 2. **面试驱动**：项目是秋招简历主力项目，每步开发同时沉淀面试素材
>
> 读者需同时遵守全局 C:\Users\10936\CLAUDE.md 的行为准则（Think Before Coding / Simplicity First / Surgical Changes / Goal-Driven Execution），本文件只补充项目特有规则，不重复全局内容。

## Part 1 开发准则

### 1.1 继承的全局准则

遵守 `C:\Users\10936\CLAUDE.md`：Think Before Coding（先陈述假设/歧义）、Simplicity First（最小代码）、Surgical Changes（只改该改的）、Goal-Driven（验证闭环）。冲突时以全局为准。

### 1.2 项目特有硬规则

| # | 规则 | 理由 |
|---|------|------|
| 1 | 数据库只读连接（`read_only=True`），永不写 olist.db | 数据是只读资产，写入会污染语义层 |
| 2 | 指标口径以 `knowledge/metrics.md` 为准，禁止自行发明定义 | 口径一致性是问数 Agent 的命根 |
| 3 | 任何 SQL 生成必须过 sqlglot 校验 + 表白名单 | 安全红线，面试必问 |
| 4 | 改 schema/口径 → 同步更新 `knowledge/schema.md` 和 `knowledge/metrics.md` | 知识库是 Agent 的唯一事实源 |
| 5 | API Key 只从 `.env` 读，绝不硬编码/打印 | 泄密红线 |
| 6 | 新增功能先查 knowledge/ 是否已有定义 | 避免重复造口径、控制 token |

### 1.3 目录与关键文件地图

```
olist_agent/
├── data/olist.db              # DuckDB 数据库（只读；由 scripts/prepare_db.py 重建）
├── knowledge/
│   ├── schema.md              # 数据字典（11 张表/视图：类型、含义、陷阱）
│   └── metrics.md             # 指标口径字典 + 7 条 SQL 模板 + 10 条生成规则
├── scripts/
│   └── prepare_db.py          # 数据库构建脚本（M1 产物）
├── docs/
│   ├── adr/                   # 架构决策记录（格式见 Part 2.2）
│   ├── interview/             # 面试素材（格式见 Part 2.2）
│   └── superpowers/           # specs/ 设计文档 + plans/ 实施计划
└── app/                       # M3 Streamlit 应用（规划中）
```

## Part 2 学习协议

本项目是秋招简历主力项目，开发必须同步沉淀面试素材。**里程碑要点文档是强制产出**（与代码同步交付），不是可选项。

### 2.1 讲解协议

**触发时机（3 类，非每步）**：

| 触发时机 | 产出 | 判断标准 |
|---------|------|---------|
| 关键架构/技术决策点 | 5-8 行短讲解 | 改变了技术选型/架构/口径 |
| 踩坑修复时 | 坑 → 根因 → 修复 → 面试话术 | 任何 bug/报错修复 |
| 里程碑完成时 | 完整面试要点文档（2.4 模板） | M2/M3 结束 |

**短讲解格式**（代码先行，讲解放本步末尾）：

```
📚 [面试要点] <知识点>
- 面试官怎么问：<问题>
- 标准回答要点：<要点>
```

**节奏约定**：短讲解 ≤8 行不刷屏；讲解不打断开发流；开发进度 > 讲解流畅性。

### 2.2 强制产出物

```
docs/adr/<YYYY-MM-DD>-<主题>.md    # 模板：日期/主题/背景/方案对比/决策/理由
docs/interview/
├── 01-项目介绍.md                # STAR 稿：1 分钟版 + 3 分钟版
├── 02-架构与流程.md              # 文字 + ASCII 架构图（面试手绘准备）
├── 03-原理问答.md                # 按主题分章：问题-答案要点-追问链
├── 04-踩坑记录.md                # 坑 → 根因 → 修复 → 面试话术
└── 05-简历项目描述.md            # 简历文案升级（STAR + 量化）
```

**更新规则**：增量更新——每次里程碑只追加新内容，不重写已有章节。

### 2.3 知识深度标准（原理级清单）

面试官对每个主题最多追 3 轮（示例追问见下表），讲解深度以此为准：

| 主题 | 必须能讲清的原理 | 3 轮追问示例 |
|------|-----------------|-------------|
| Agent 基础 | ReAct 循环、Agent vs Workflow、Function Calling 机制、工具调用失败重试/死循环防护 | Function Calling 是怎么被 LLM 触发执行的？ |
| RAG | 四步链路（切分→嵌入→检索→生成）、向量检索（ANN/HNSW）、混合检索+重排、幻觉控制 | HNSW 的原理是什么？为什么快？ |
| Text2SQL | 难点（schema 超大/口径歧义/多表 join）、AST 校验原理、安全设计 | 怎么防止 LLM 生成危险 SQL？ |
| 编排 | LangGraph 状态图（节点/边/状态/checkpoint）、Chain vs Agent 区别 | 为什么用状态图而不是简单的链式调用？ |
| 评测 | EX 执行准确率、Spider/BIRD 基准、坏例→few-shot 数据飞轮 | 你怎么量化 Agent 的准确率？ |
| 工程化 | 会话记忆、Streamlit 部署、API Key 安全管理 | 多轮对话怎么保持上下文？ |

### 2.4 里程碑面试要点模板（五段式）

```
## M<x> 面试要点（日期）
### 做了什么（STAR，3 句）
### 为什么这么做（决策 + 理由）
### 技术亮点（2-3 个可吹的点）
### 可能被问的 5 个问题（含标准答案要点）
### 本项目能延伸到的原理（链接到 2.3 清单）
```

## 运行约定

- 本文件与全部面试文档**全中文**
- 讲解时机由 Claude 自行判断（判断标准见 2.1）
- 新增产出物（ADR/面试要点）后立即 commit（git）
