...
**面试叙事**：成本优化——固定 prompt 前缀利用服务端上下文缓存。设计上把 system prompt 组织为"稳定前缀 + 可变增量"（RAG 尾部追加而非插入），使每轮调用的前 4.6K tokens 完全一致，白嫖百炼隐式缓存命中（92% 输入按 20% 计费）。**方法论教训**：验证缓存必须先用官方文档确认生效条件（1024-token 门槛），否则小前缀的负结果是没有信息量的假阴性——"没验证门槛就下结论"是最贵的错误。
...
**价值**：安全设计补齐"事后追溯"半环——事前有四道闸（AST 校验/白名单/LIMIT/超时），事后有全量审计链；演示与面试可直接打开日志文件证明"Agent 每步干了什么"，与只读连接形成完整可审计闭环。
