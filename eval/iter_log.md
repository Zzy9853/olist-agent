...
| Q04/Q14 ROUND 精度（118.57 vs 118.573） | 严格容差 1e-6 不认精度差异 | 容差放宽到 1e-2（仅兜精度）；Q01 尺度问题由 prompt 铁律（不乘 100）修行为 |
...
- Q17：LLM 采样波动（生成的 DATE_TRUNC + COUNT(*) 形态与 ref 细节不一致，具体差异无法复现）；**4 次复测全 PASS**（DATE_TRUNC 标准写法），非重构回归
- Q18：历史已知采样波动（多次评测均偶发，LLM 漏 valid_orders 过滤）
**结论：重构无系统性回归**（失败均为独立采样波动，非提示词组织方式变化导致）。test_agent 5/5、workflow、Judge 区分度全过。
...
**验证目标**：确认百炼隐式上下文缓存对项目真实调用生效（固定 system prompt 前缀 → 命中减免计费）。

**验证过程**（实测输出）：
1. **首次测试（无效）**：任务模板脚本用 319-token 前缀（'你是固定前缀测试。'×50）连调两次，两次均 `cached_tokens=0`、响应头无缓存标记。**判定为测试无效**：官方门槛为最小可缓存提示词 ≥1024 tokens（百炼 Context Cache 文档），319 < 1024，不满足缓存条件，"0"不能证明不支持。
2. **真实场景测试（命中）**：用 `build_system_prompt()` 真实 system prompt（4595 tokens）连调两次——第 1 次 `cached_tokens=0`（冷启动），第 2 次 **`cached_tokens=4224`**；复测 3 次均稳定 4224（5 分钟 ephemeral 有效期窗口内）。
3. **结构确认**：`build_system_prompt` 为固定四层前缀（角色/知识/规则/契约）+ RAG `extra_context` 追加在**末尾**（prompts.py:79-93）——RAG 动态内容不破坏固定前缀，生产链路（每轮 system prompt 前 4595 tokens 完全一致）同样命中。

**结论：百炼隐式缓存命中（无需改代码）**。隐式模式默认开启（服务端自动识别重复前缀，无需请求头），命中部分按标准输入单价 **20%** 计费（官方：`cached_tokens` 计费参数查看命中 token 数；显式 `cache_control: ephemeral` 命中 10%/创建 125%，但隐式已满足，不引入配置）。成本测算：每轮输入中 ~4224/4595 ≈ 92% 为命中缓存，该部分仅按 20% 计费——输入成本约为原始的 26%（0.92×0.2 + 0.08×1.0），输出不变；本项目输入占绝对主导（4.6K prompt vs 短输出），整体调用成本显著下降。收益前提是缓存 5 分钟有效期内前缀稳定——单会话连续多轮问数天然满足。

**要点**：成本优化——固定 prompt 前缀利用服务端上下文缓存。设计上把 system prompt 组织为"稳定前缀 + 可变增量"（RAG 尾部追加而非插入），使每轮调用的前 4.6K tokens 完全一致，白嫖百炼隐式缓存命中（92% 输入按 20% 计费）。**方法论教训**：验证缓存必须先用官方文档确认生效条件（1024-token 门槛），否则小前缀的负结果是没有信息量的假阴性——"没验证门槛就下结论"是最贵的错误。

---

# 项目增强（2026-08-10）

> 流式输出 / Judge 抗偏 / SQL 审计日志 / 上下文缓存 —— 四件产品化增强，全部实测验证，回归全绿（6 套测试全过 + 评测冒烟 3/3；同步 ask 路径未变，流式为纯增量入口）。

## 增强 1：流式输出（ask_stream 双通道）

**设计**：`GRAPH.astream(stream_mode=["updates", "custom"])` 双通道——`updates` 通道携带各节点状态（按节点累计，done 事件据此提取完整最终状态）；`custom` 通道由节点内 `langgraph.config.get_stream_writer()` 写出解读 token。**JSON 生成保持非流式**（gen_sql 结构化输出仍走 chat_json，解析稳定不随采样波动），**解读流式**——JSON 生成完毕后首 token 秒回，UI 边收边渲染。

**事件协议**（`app/agent.py` ask_stream，UI 专用入口；评测/MCP 仍走同步 ask，零行为变化）：

| 事件 | 载荷 | 语义 |
|------|------|------|
| `{"type": "stage"}` | `node` + 中文 `label`（_STAGE_NAMES：检索知识/生成 SQL/执行查询/生成归因…） | 节点阶段流转，UI 映射为 st.status |
| `{"type": "token"}` | `content`（解读增量） | 逐 token 追加渲染 |
| `{"type": "done"}` | `answer`/`sql`/`attribution`/`workflow_result` | 完整最终状态，异常路径也保证 done |

**实测（接线关键，工程教训）**：langgraph 1.2 多模式 astream 的事件形状实测为 **`(mode, payload)` 二元组**——`async for mode, payload in GRAPH.astream(...)` 逐个解包，按 mode 分流（custom→token 流，updates→状态累计）；同一节点重跑（SQL 校验失败重试）时 updates 覆盖旧值，done 汇总的是最终状态。

## 增强 2：Judge 抗偏（Rubrics 条款 + 双评均值）

**论文依据**：LLM-as-Judge 偏置研究（如 "Large Language Models are not Fair Evaluators"）已量化多种偏置——**agreeableness（迎合性）偏置**（模型倾向认可问题前提，评分随语气走）与**长度偏见**（verbosity bias，越长越华丽得分越高，与事实准确性无关）。这些偏置会把"自洽的错误"评成高分——本项目双轨评测早前发现的 Q18 案例（内部自洽但执行不一致，无参考 Judge 给满分）正是其现实样本。

**抗偏设计（两层）**：
1. **Rubrics 抗偏条款**（`app/prompts.py` RUBRICS 末行）：「评分只依据事实准确性与洞察质量——不因回答长度、措辞华丽、结构美观加分，不因简洁扣分」——把量化过的偏置风险转成评分约束。
2. **双评均值**（`app/judge.py`）：同一问答对两次独立评估——temperature 0.0（确定性基线）+ 0.3（多样化复核），各维度取均值（reasoning 保留低温度版本），评测默认启用双评，降低单次采样波动对评分的影响。

## 增强 3：SQL 执行审计日志（每次执行可追溯）

**设计**：`app/executor.py` 每次执行落盘 `data/audit.log` 一条：`[时间] status=ok|err rows=行数 elapsed=耗时ms sql=<摘要前100字符> [error=<错误摘要>]`。审计失败不影响主流程（OSError 吞掉），只读执行器行为零变化。

**价值**：安全设计补齐"事后追溯"半环——事前有四道闸（AST 校验/白名单/LIMIT/超时），事后有全量审计链；演示可直接打开日志文件证明"Agent 每步干了什么"，与只读连接形成完整可审计闭环。

## 增强 4：上下文缓存成本优化

**实证**（详见上文「上下文缓存验证（2026-08-10）」小节）：真实 system prompt 4595 tokens，第二次调用起 `cached_tokens=4224` 稳定命中——约 92% 输入走缓存（命中部分按 20% 单价），输入成本降至约 26%；**"稳定前缀 + RAG 尾部追加"结构天然适配**——动态检索内容追加在末尾不破坏固定前缀，生产链路每轮同样命中。
...
**教训**：图表组件选型是"渲染层工程"——Streamlit 原生图表（vega-lite）降级残留、plotly 跨轴语义、元素 ID 机制，都是真实环境才暴露的坑；图表链路需三层防御：组件选型（plotly 替代 vega-lite）、布局语义（跨轴不可并排）、ID 管理（唯一 key）。
